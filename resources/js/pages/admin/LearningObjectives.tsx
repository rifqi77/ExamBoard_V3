...
// Admin Curriculum (Learning Objectives) — school-wide. Identical to the
// teacher curriculum manager (four curriculum tabs, server-side Excel
// parse → preview → confirm import, collapsible topic → subtopic → LO tree
// with inline add / edit / delete + multi-select bulk delete), but
// unscoped: every uploaded LO across teachers. Paths point at
...
export default function AdminLearningObjectives() {
    ...
    const [activeTab, setActiveTab] = useUIState<CurriculumKey>('admin.lo.activeTab', 'kurikulum_merdeka');
    const [language, setLanguage] = useUIState<string>('admin.lo.language', 'Indonesia');
    const [subject, setSubject] = useUIState<string>('admin.lo.subject', acctSubject || '');
    const [topicFilter, setTopicFilter] = useUIState<string>('admin.lo.topicFilter', '');
    ...
    const [openTopics, setOpenTopics] = useUIState<string[]>('admin.lo.openTopics', []);
    const [openSubtopics, setOpenSubtopics] = useUIState<string[]>('admin.lo.openSubtopics', []);
    ...
    function onFileSelected(event: React.ChangeEvent<HTMLInputElement>) {
        ...
        event.target.value = '';
        if (!file) return;
        // Client-side sanity check — clearer than Laravel's silent 422 bounce.
        ...
        setBusy(true);
        router.post(
            '/admin/learning-objectives/upload',
    ...
    // Inline error surfaced under the Import button when validation fails
    // (replaces the old silent `return` that left users wondering why the
    // button "did nothing"). Cleared automatically once the issue resolves.
    ...
    function onCancelPreview() {
        router.get('/admin/learning-objectives', {}, { preserveScroll: true, preserveState: false });
    }

    function onDelete(id: string) {
        if (!confirm('Delete this learning objective? This cannot be undone.')) return;
        router.delete(`/admin/learning-objectives/${encodeURIComponent(id)}`, { preserveScroll: true });
    }
    ...
    function onBulkDelete() {
        ...
        setBusy(true);
        router.post(
            '/admin/learning-objectives/bulk-delete',
    ...
    function submitEdit() {
        ...
        setBusy(true);
        router.patch(
            `/admin/learning-objectives/${encodeURIComponent(editing.id)}`,
            {
        ...
        <AdminShell>
            ...
            <header className="teacher-page-header">
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'flex-end' }}>
                    ...
                    {/* Visible destination hint — prevents the #1 source of "the
                        import didn't work" reports (file imported into a different
                        curriculum tab than the user expected to find it under). */}
                ...
                <div className="section-title-row">
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
                        <button
                            ...
                            onClick={() => setAdding(adding ? null : { topic: '', subtopic: '', text: '' })}
                            style={{ width: 'auto' }}
                        >
