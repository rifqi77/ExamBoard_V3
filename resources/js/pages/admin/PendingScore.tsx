...
import { ChevronDown, ChevronRight, ExternalLink, FileDown, Upload, Users } from 'lucide-react';
import { useMemo, useState } from 'react';
import AdminShell from '@/components/AdminShell';
import { useUIState } from '@/lib/useUIState';

// Admin Pending Score — school-wide. The teacher Exam → Class → Students
// tree restricted to submissions with a pending essay, spanning every
// teacher's exams, with a per-teacher scope picker. Each exam (and each
// student) exports a self-contained Markdown bundle for an AI assistant;
// the JSON it returns is pasted back via "Import AI scores" which POSTs to
// /admin/grade-bulk. Grading View links go to /admin/scores/[submissionId].
...
export default function AdminPendingScore() {
    const { groups, aiExports, teachers, teacherId, flash } = usePage().props as unknown as {
        groups: ExamGroupT[];
        aiExports: AiExports;
    ...
    const [openExamIds, setOpenExamIds] = useUIState<string[]>('admin.pending-score.openExams', []);
    const [openClassKeys, setOpenClassKeys] = useUIState<string[]>('admin.pending-score.openClasses', []);
    ...
    function setTeacher(next: string) {
        router.get('/admin/pending-score', next ? { teacherId: next } : {}, {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
    ...
    return (
        <AdminShell>
            <Head title="Admin · Pending Score" />
            <header className="teacher-page-header">
                <div>
                    <h1>Pending Score</h1>
                    <p>
                        Essays still waiting for manual grading, across every teacher&apos;s exams. Click View to grade each
                        essay alongside the mark scheme, or Export to bundle answers for an AI assistant.
                    </p>
                </div>
            </header>
                            ...
                            <div className="section-title-row">
                                ...
                                {focusedCount > 0 && examBundle ? (
                                    <button
                                        ...
                                        style={{ width: 'auto' }}
                                        onClick={() => copyOrDownload(`${exam.examId}-ai-grading.md`, examBundle, setNotice)}
                                          ...
                                          <div className="scores-class-group" key={cls.classId ?? 'no-class'}>
                                              <div className="scores-class-header">
                                                  ...
                                                  <span className="scores-class-toggle-label">
                                                      ...
                                                      <span className="scores-class-meta">{cls.submissions.length} pending</span>
                                                  </span>
                                              </div>
                                              {classOpen ? (
...
// Admin pastes the JSON the AI returned; we POST it to /admin/grade-bulk
// (which delegates to the shared grader — admin may grade any teacher's
// submission) and reload so freshly-graded rows drop out of "pending".
function ImportAIScoresPanel() {
    ...
    async function submit() {
        ...
        setBusy(true);
        try {
            const res = await fetch('/admin/grade-bulk', {
                method: 'POST',
                headers: {
