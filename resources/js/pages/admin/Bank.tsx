...
// Admin Question Bank — school-wide. Identical UI to the teacher bank
// (subject→topic→subtopic→difficulty→media tree, 6 filters + search,
// inline create / edit / delete, zip/xlsx upload), but unscoped: every
// teacher's contributions, all manageable. Paths point at /admin/bank*.
...
export default function AdminBank() {
    ...
    // Tree expansion state — persisted as string arrays so JSON-based storage
    // round-trips correctly (Set<string> would serialize to `{}` and lose all
    // entries on reload).
    const [openSubjects, setOpenSubjects] = useUIState<string[]>('admin.bank.openSubjects', []);
    const [openTopics, setOpenTopics] = useUIState<string[]>('admin.bank.openTopics', []);
    const [openSubtopics, setOpenSubtopics] = useUIState<string[]>('admin.bank.openSubtopics', []);
    const [openDifficulties, setOpenDifficulties] = useUIState<string[]>('admin.bank.openDifficulties', []);
    const [openMediaGroups, setOpenMediaGroups] = useUIState<string[]>('admin.bank.openMediaGroups', []);
    const [openQuestions, setOpenQuestions] = useUIState<string[]>('admin.bank.openQuestions', []);
    ...
    function setFilter(key: keyof Filters, value: string) {
        ...
        router.get('/admin/bank', next, {
            preserveState: true, preserveScroll: true, replace: true,
        });
    }
    function clearFilters() {
        router.get('/admin/bank', {}, { preserveState: true, preserveScroll: true, replace: true });
    }
    ...
    function onDelete(id: string) {
        ...
        router.delete(`/admin/bank/${id}`, { preserveScroll: true });
    }
    ...
    return (
        <AdminShell>
            <Head title="Admin · Question Bank" />
            <header className="teacher-page-header">
                <div>
                    ...
                    <p>
                        ...
                        {isAdmin
                            ? ' Shared bank — admin database (every teacher\'s contributions).'
                            : ' Read-only shared bank.'}
                    </p>
                </div>
                ...
                <section className="admin-panel">
                    <div className="section-title-row">
                        <div>
                            ...
                            <p>Added to the shared admin database.</p>
                        </div>
                    </div>
                    <QuestionForm
                        mode="create"
...
function UploadButton() {
    ...
    function onFile(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        if (!file) return;
        setBusy(true);
        router.post('/admin/bank/upload', { file }, {
            forceFormData: true,
            preserveScroll: true,
            onFinish: () => {
                setBusy(false);
