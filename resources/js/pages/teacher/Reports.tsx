...
// ---------------------------------------------------------------------------
// Teacher Reports — port of TeacherReportsClient.tsx. The report matrix is
// computed server-side (ReportsController@index) and passed in as `report`;
// the page renders the per-class collapsible score tables + drives the .xlsx
// export (ReportsController@export) via a native form POST so the browser
...
type TeacherReport = { exams: ReportExamColumn[]; classes: ReportClassGroup[] };
...
export default function Reports() {
    const { auth, report, currentAcademicYear } = usePage().props as unknown as {
        auth: { user?: { fullName?: string } };
        report: TeacherReport;
        currentAcademicYear: string;
    };

    const [columns, setColumns] = useUIState<ReportColumnSelection>('teacher.reports.columns', DEFAULT_COLUMN_SELECTION);
    const [openClasses, setOpenClasses] = useUIState<string[]>('teacher.reports.openClasses', []);
    const [yearFilter, setYearFilter] = useUIState<string>('teacher.reports.yearFilter', currentAcademicYear);
    ...
    // Build + submit a native form POST so the browser downloads the .xlsx.
    // Same-origin form POST carries cookies + a matching Origin/Referer, so
    // session auth + the Origin-based CSRF check both pass. The response is a
    // streamed download (Content-Disposition), so targeting a hidden iframe
    ...
    function doExport(targetClassIds: Array<string | null>, label: string, filename: string) {
        ...
        try {
            ...
            form.method = 'POST';
            form.action = '/teacher/reports/export';
            ...
            // Empty list = "export all" on the server; for an explicit single
            // class we send its key (or __no_class__ for the orphan bucket).
            targetClassIds.forEach((id) => append('classIds[]', classKey(id)));
            (Object.keys(columns) as ReportColumnKey[]).forEach((k) =>
                append(`columns[${k}]`, columns[k] ? '1' : '0'),
            );
            append('label', label);
    ...
    // Distinct academic years across loaded classes, newest-first. The
    // "No class" bucket (academicYear === null) is excluded here.
    const availableYears = useMemo(
        () =>
            Array.from(new Set((report?.classes ?? []).map((c) => c.academicYear).filter((y): y is string => Boolean(y)))).sort(
                (a, b) => b.localeCompare(a),
            ),
        [report],
    );

    // Year filter; the "No class" bucket is always kept so orphan students
    // stay visible regardless of the selected year.
        ...
        <TeacherShell>
            ...
            <header className="teacher-page-header">
                <div>
                    <h1>Reports</h1>
                    <p>
                        Student achievement and progress across your exams, grouped by class. Export to Excel for
