...
import { KeyRound, Plus, RefreshCw, Sparkles, Trash2, Upload, X } from 'lucide-react';
...
export default function Exams() {
    ...
    const isAdmin: boolean = props.isAdmin ?? false;
    const flash = props.flash ?? {};
    const caps = props.auth?.user?.capabilities ?? {};
    const hasCap = (key: string) => caps[key] === true || (typeof caps[key] === 'number' && caps[key] > 0);
    const canAi = isAdmin || hasCap('ai.generate');
    ...
    function onFileSelected(event: React.ChangeEvent<HTMLInputElement>) {
        ...
        if (!file) return;
        // Auto-submit as multipart — server-side parse + import in one round
        // trip. useForm tracks `processing` so the button reflects the upload.
        importForm.setData('package', file);
        importForm.post('/teacher/exams/import', {
            forceFormData: true,
            preserveScroll: true,
            onFinish: () => {
    ...
    function onRegenerateToken(token: TokenSummary) {
        ...
        router.post(`/teacher/exams/tokens/${token.id}/regenerate`, {}, {
            preserveScroll: true,
    ...
    function onDeleteToken(token: TokenSummary) {
        ...
        setBusyTokenId(token.id);
        router.delete(`/teacher/exams/tokens/${token.id}`, {
            preserveScroll: true,
            onFinish: () => setBusyTokenId(null),
        });
    }
    ...
    function onDeleteExam(exam: ExamSummary) {
        ...
        router.delete(`/teacher/exams/${exam.examId}`, {
            preserveScroll: true,
    ...
    return (
        <TeacherShell>
            <Head title="Teacher · Exams" />
            <header className="teacher-page-header">
                <div>
                    <h1>Exams</h1>
                    <p>Click an exam to manage questions, tokens, and submissions.</p>
                </div>
                <div>
                    ...
                    {canAi ? (
                        <Link className="ghost-button" href="/teacher/ai-generate">
                            <Sparkles size={17} aria-hidden /> Generate with AI
                        </Link>
                    ) : null}
                    <Link className="primary-button" href="/teacher/exams/new">
                        ...
                        <tbody>
                            {exams.map((exam) => (
                                <tr key={exam.examDatabaseId}>
                                    <td>
                                        <Link
                                            href={`/teacher/exams/${exam.examId}`}
                                    ...
                                    <td>
                                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                                            <Link className="ghost-button" href={`/teacher/exams/${exam.examId}`}>
                                                Manage
                                            </Link>
                                            <button
                                                type="button"
